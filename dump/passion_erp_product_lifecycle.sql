...
CREATE TABLE `product_lifecycle` (
  ...
  `barcode` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Product barcode for tracking',
  `batch_number` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Batch number for this product instance',
  `serial_number` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Unique serial number for this product instance',
  `current_stage` enum('created','material_allocated','in_production','cutting','embroidery','printing','stitching','finishing','ironing','quality_check','packing','ready_for_dispatch','dispatched','in_transit','delivered','returned','rejected') COLLATE utf8mb4_unicode_ci DEFAULT 'created',
  `current_status` enum('active','on_hold','completed','cancelled','returned') COLLATE utf8mb4_unicode_ci DEFAULT 'active',
  ...
  `location` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Current physical location of the product',
  ...
  `quality_status` enum('pending','passed','failed','rework_required') COLLATE utf8mb4_unicode_ci DEFAULT 'pending',
  ...
  `qr_token` varchar(120) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Unique token identifying the lifecycle QR',
  `qr_payload` json DEFAULT NULL COMMENT 'Cached payload embedded within the lifecycle QR',
  `qr_status` enum('pending','active','revoked','expired') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'pending' COMMENT 'Status of the lifecycle QR token',
