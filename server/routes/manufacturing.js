...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // ✅ NEW: Enhance product names from multiple sources if not linked to Product table
      const enrichedOrders = productionOrders.map((order) => {
        const orderObj = order.toJSON();

        // If product name is missing, try to get it from alternative sources
        if (!orderObj.product?.name) {
          let productName = null;

          // 1. Try to get from Sales Order specifications
          if (orderObj.salesOrder) {
            const soObj =
              typeof orderObj.salesOrder === "object"
                ? orderObj.salesOrder
                : {};

            // Check garment_specifications
            if (soObj.garment_specifications) {
              const specs =
                typeof soObj.garment_specifications === "string"
                  ? JSON.parse(soObj.garment_specifications)
                  : soObj.garment_specifications;
              productName = specs?.product_type || specs?.product_name;
            }

            // Check items array
            if (!productName && soObj.items) {
              const items =
                typeof soObj.items === "string"
                  ? JSON.parse(soObj.items)
                  : soObj.items;
              if (Array.isArray(items) && items.length > 0) {
          ...
          // 2. Try to get from specifications JSON field
          if (!productName && orderObj.specifications) {
            const specs =
              typeof orderObj.specifications === "string"
                ? JSON.parse(orderObj.specifications)
                : orderObj.specifications;
            productName = specs?.product_name || specs?.product_type;
          }

          // If still not found, create a composite display
          if (!productName && orderObj.project_reference) {
            productName = `Project: ${orderObj.project_reference}`;
          }
          ...
          if (productName) {
            ...
            orderObj.product.id = orderObj.product_id;
