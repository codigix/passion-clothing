...
router.post('/pos/:id/submit-for-approval', authenticateToken, checkDepartment(['procurement', 'admin']), async (req, res) => {
  ...
  try {
    ...
    await NotificationService.sendToDepartment('admin', {
      ...
      message: `Purchase Order ${purchaseOrder.po_number} has been submitted for approval. Vendor: ${purchaseOrder.vendor?.name}, Amount: ₹${purchaseOrder.final_amount?.toLocaleString()}`,
