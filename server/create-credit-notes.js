...
const createCreditNotesTable = async () => {
  try {
    ...
    console.log('Checking if credit_notes table exists...');
    const tableExists = await queryInterface.tableExists('credit_notes');
    
    if (tableExists) {
      console.log('credit_notes table already exists');
      process.exit(0);
    }
    
    console.log('Creating credit_notes table...');
    ...
    console.log('✅ credit_notes table created successfully!');
    process.exit(0);
  } catch (err) {
