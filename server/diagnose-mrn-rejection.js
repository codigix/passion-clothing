...
const { Inventory, Product, ProjectMaterialRequest, sequelize } = require('./config/database');
...
async function diagnoseMRNRejection() {
  try {
    ...
    const inventoryWithProduct = await Inventory.findAll({
      ...
      attributes: ['id', 'product_name', 'category', 'current_stock', 'available_stock', 
                  'unit_of_measurement', 'quality_status', 'is_active', 'product_id', 'barcode'],
      limit: 10,
      order: [['created_at', 'DESC']]
    });
    ...
    inventoryWithProduct.forEach((item, idx) => {
      ...
      console.log(`      Available Stock: ${item.available_stock || 0} ${item.unit_of_measurement}`);
      console.log(`      Current Stock: ${item.current_stock || 0} ${item.unit_of_measurement}`);
    ...
    if (latestMRN && latestMRN.materials_requested && latestMRN.materials_requested.length > 0) {
      ...
      if (searchResult.length === 0) {
        ...
        const altSearch = await Inventory.findAll({
          ...
          attributes: ['id', 'product_name', 'current_stock', 'available_stock', 'product_id', 'quality_status', 'is_active', 'unit_of_measurement']
        });
        
        console.log(`   Found ${altSearch.length} items by name match:`);
        altSearch.forEach(item => {
          console.log(`      - ${item.product_name}: ${item.available_stock || item.current_stock || 0} ${item.unit_of_measurement}`);
          console.log(`        Product ID: ${item.product_id || 'NULL ❌'}`);
