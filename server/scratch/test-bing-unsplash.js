...
async function testBingUnsplash(query) {
  try {
    ...
    while ((match = regex.exec(html)) !== null) {
      ...
      if (!imgUrls.includes(imgUrl) && imgUrl.includes('unsplash.com')) {
        imgUrls.push(imgUrl);
      }
    }
    
    console.log(`Found ${imgUrls.length} Unsplash images!`);
...
async function run() {
  await testBingUnsplash("pink floral clothing patterns");
  await testBingUnsplash("women polo t-shirt");
}
